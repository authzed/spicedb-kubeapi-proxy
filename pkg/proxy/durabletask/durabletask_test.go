...
func TestDurableTask(t *testing.T) {
	...
	scheduler, worker, err := Setup(psc, kubeClient, MemorySQLite)
	require.NoError(t, err)
	...
	id, err := scheduler.Schedule(ctx, distributedtx.DefaultLockMode, &distributedtx.CreateObjInput{
	...
	require.NoError(t, err)
	require.Empty(t, metadata.Err, "workflow returned error: %s", metadata.Err)

	resp, err := psc.CheckPermission(ctx, &v1.CheckPermissionRequest{
